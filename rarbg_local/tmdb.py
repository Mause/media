...
class TmdbAPI:
    def __init__(self, settings: Annotated[Settings, Depends(get_settings)]):
        ...
        self.access_token = os.environ['TMDB_READ_ACCESS_TOKEN']
    ...
    async def get_json(self, path, **kwargs):
        async with aiohttp.ClientSession(
            base_url=base,
            headers={
                'Authorization': f'Bearer {self.settings.tmdb_read_access_token}',
            },
        ) as tmdb:
    ...
    async def search_themoviedb(self, s: str) -> list[SearchResponse]:
        MAP = {'tv': MediaType.SERIES, 'movie': MediaType.MOVIE}
        r = await self.get_json('search/multi', params={'query': s})
        return [
            SearchResponse(
    ...
    @cached(TTLCache(1024, 360))
    async def find_themoviedb(self, imdb_id: ImdbId) -> dict[str, str]:
        precondition(imdb_id.startswith('tt'), 'Invalid imdb_id')
        results = await self.get_json(
    ...
    async def resolve_id(self, imdb_id: ImdbId, type: ThingType) -> TmdbId:
        precondition(imdb_id.startswith('tt'), 'Invalid imdb_id')
        results = await self.get_json(
            f'find/{imdb_id}', params={'external_source': 'imdb_id'}
        )
    ...
    @cached(LRUCache(256))
    async def get_movie(self, id: TmdbId) -> MovieResponse:
        return MovieResponse.model_validate(await self.get_json(f'movie/{id}'))

    @cached(TTLCache(256, 360))
    async def get_tv(self, id: TmdbId) -> TvApiResponse:
        return TvApiResponse.model_validate(await self.get_json(f'tv/{id}'))

    async def get_movie_imdb_id(self, movie_id: TmdbId) -> ImdbId:
        return await self.get_imdb_id('movie', movie_id)

    async def get_tv_imdb_id(self, tv_id: TmdbId) -> ImdbId:
        return await self.get_imdb_id('tv', tv_id)

    @cached(LRUCache(360))
    async def get_imdb_id(self, type: ThingType, id: TmdbId) -> ImdbId:
        return (await self.get_json(f'{type}/{id}/external_ids'))['imdb_id']

    @cached(TTLCache(256, 360))
    async def get_tv_episodes(self, id: TmdbId, season: int) -> TvSeasonResponse:
        return TvSeasonResponse.model_validate(
            await self.get_json(f'tv/{id}/season/{season}')
        )

    async def discover(self, types=(ReleaseType.PHYSICAL, ReleaseType.DIGITAL)):
